...
function Ping_check() {
    ...
    if  [ $networksum -eq 0 ];then
        ...
        # 输出开始测试的容器 ID
        LOG_WARN "[${step_flag}/${step_total}]\t开始测试业务容器 IPv4 连通性..."
        step_flag=$(( ${step_flag} + 1 ))
        # 测试 IPv4 连通性
        if docker exec "$podid" sh -c "ping -c 1 www.baidu.com" > /dev/null 2>&1; then
            LOG_INFO "\t容器 $podid IPv4 连通性测试成功。"
        else
            LOG_ERROR "\t容器 $podid IPv4 连通性测试失败。（该结果为随机抽取network测试，不代表全部容器）"
        fi
    fi
}
...
function Ping6_check() {
    ...
    if  [ $networksum -eq 0 ];then
        ...
        podid=${containers[$random_index]}
        # 输出开始测试的容器 ID
        LOG_WARN "[${step_flag}/${step_total}]\t开始测试业务容器 IPv6 连通性..."
        step_flag=$(( ${step_flag} + 1 ))
        # 测试 IPv6 连通性
