...
if [ "$show_help" = true ]; then
    echo "用法: $0 [选项]"
    echo "选项:"
    echo "  --skip-ping | -skip-ping    跳过 Ping 检查（Ping_check 和 Ping6_check）"
...
function Ping_check() {
    ...
    if  [ $networksum -eq 0 ];then
        ...
        LOG_WARN "[${step_flag}/${step_total}]\t开始测试业务容器 IPv4 连通性，可能比较耗时，请等待..."
        step_flag=$(( ${step_flag} + 1 ))
...
function Ping6_check() {
    ...
    if  [ $networksum -eq 0 ];then
        ...
        LOG_WARN "[${step_flag}/${step_total}]\t开始测试业务容器 IPv6 连通性，可能比较耗时，请等待..."
        step_flag=$(( ${step_flag} + 1 ))
...
# 根据选项决定是否执行 Ping 检查
if [ "$skip_ping_check" = false ]; then
    Ping_check
    Ping6_check
else
    LOG_WARN "[${step_flag}/${step_total}]\t跳过 Ping 检查"
    step_flag=$(( ${step_flag} + 1 ))
fi